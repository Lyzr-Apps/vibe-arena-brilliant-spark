...
export default function Home() {
  ...
  // ─── Report Data (used inline in report screen) ────
  const reportData = vibeReport || (showSampleData ? SAMPLE_VIBE_REPORT : null)
  const reportBreakdown = reportData?.score_breakdown
            ...
            <div className="space-y-4 animate-fade-in-up">
              ...
              <Card className={`glass-effect ${cardBorderClass}`}>
                <CardContent className="pt-6 pb-4 flex flex-col items-center">
                  <ScoreDisplay score={analystResult.overall_score ?? 0} label={analystResult.score_label} mode={currentMode} />
                  <p className="text-sm text-muted-foreground mt-3 text-center max-w-md">{analystResult.summary}</p>
                </CardContent>
              </Card>
              ...
              <Card className={`glass-effect ${cardBorderClass}`}>
                ...
                <CardContent>
                  <VibeBar label="Romance" value={analystResult.vibe_meter?.romance ?? 0} color="#FF6496" />
                  <VibeBar label="Toxicity" value={analystResult.vibe_meter?.toxicity ?? 0} color="#FF4444" />
                  <VibeBar label="Communication" value={analystResult.vibe_meter?.communication ?? 0} color="#00FFFF" />
                  <VibeBar label="Effort" value={analystResult.vibe_meter?.effort ?? 0} color="#E600E6" />
                  <VibeBar label="Compatibility" value={analystResult.vibe_meter?.compatibility ?? 0} color="#FFFF00" />
                </CardContent>
              </Card>
              ...
              <Card className={`glass-effect ${cardBorderClass}`}>
                ...
                <CardContent className="space-y-3">
                  {Array.isArray(analystResult.findings) && analystResult.findings.map((finding, i) => (
              ...
              <Card className={`glass-effect ${cardBorderClass}`}>
                <CardContent className="pt-4 space-y-3">
                  <div>
                    ...
                    <p className="text-sm mt-1" style={{ color: accentColor }}>{analystResult.verdict}</p>
                  </div>
                  <Separator className="opacity-20" />
                  <div>
                    <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Recommendations</span>
                    <ul className="mt-1 space-y-1">
                      {Array.isArray(analystResult.recommendations) && analystResult.recommendations.map((rec, i) => (
                  ...
                  <div className="flex gap-2 pt-2">
                    <CopyButton text={analystResult.receipts || analystResult.verdict || ''} label="Copy Receipt" />
            ...
            <div className="max-w-4xl mx-auto px-4 py-6 space-y-6 relative z-10">
              ...
              <Card className={`glass-effect ${cardBorderClass} ${pulseClass}`}>
                <CardContent className="py-8 flex flex-col items-center">
                  <ScoreDisplay score={reportData.vibe_score ?? 0} label="Overall Vibe Score" mode={currentMode} />
                  {reportData.personality_type && (
              ...
              {reportBreakdown && (
                <Card className={`glass-effect ${cardBorderClass}`}>
                  ...
                  <CardContent>
                    <VibeBar label="Romance Quotient" value={reportBreakdown.romance_quotient ?? 0} color="#FF6496" />
                    <VibeBar label="Independence Index" value={reportBreakdown.independence_index ?? 0} color="#00FFFF" />
                    <VibeBar label="Creativity Spark" value={reportBreakdown.creativity_spark ?? 0} color="#FFFF00" />
                    <VibeBar label="Debate Prowess" value={reportBreakdown.debate_prowess ?? 0} color="#E600E6" />
                  </CardContent>
                </Card>
              )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {reportData.spirit_animal && (
                      <div className="text-center p-3 rounded" style={{ background: 'rgba(255,255,255,0.03)' }}>
                        ...
                        <p className="text-sm font-bold" style={{ color: '#FFFF00' }}>{reportData.spirit_animal}</p>
                      </div>
                    )}
                    {reportData.playlist_mood && (
                      <div className="text-center p-3 rounded" style={{ background: 'rgba(255,255,255,0.03)' }}>
                        <span className="text-[10px] uppercase tracking-widest text-muted-foreground block mb-1">Playlist Mood</span>
                        <p className="text-sm font-bold" style={{ color: '#E600E6' }}>{reportData.playlist_mood}</p>
                      </div>
                    )}
                    {reportData.viral_caption && (
                      <div className="text-center p-3 rounded" style={{ background: 'rgba(255,255,255,0.03)' }}>
                        <span className="text-[10px] uppercase tracking-widest text-muted-foreground block mb-1">Viral Caption</span>
                        <p className="text-xs italic" style={{ color: accentColor }}>{reportData.viral_caption}</p>
                      </div>
                    )}
                  </div>
                </CardContent>
              ...
              {reportData.final_verdict && (
                <Card className={`glass-effect ${cardBorderClass}`} style={{ background: `linear-gradient(135deg, rgba(0,255,255,0.05), rgba(230,0,230,0.05))` }}>
                  <CardContent className="py-4 text-center">
                    ...
                    <p className="text-sm leading-relaxed max-w-lg mx-auto" style={{ color: accentColor }}>{reportData.final_verdict}</p>
                  </CardContent>
                </Card>
              )}
              ...
              <div className="flex justify-center gap-2">
                <CopyButton
                  text={`VibeSplit Vibe Report\nScore: ${reportData.vibe_score ?? 0}/100\nPersonality: ${reportData.personality_type ?? ''}\n${reportData.viral_caption ?? ''}\n${reportData.final_verdict ?? ''}`}
                  label="Share Report"
                />
              </div>
            </div>
